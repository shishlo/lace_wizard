...
from .statistics import calculateAvgErr
...
class HramonicsFunc:
	...
	def findMin(self):
		...
		phase_min =  phaseNearTargetPhaseDeg(-self.param_arr[2] + 180.,0.)
		phase_step = 20.
		delta = 0.001
	...
	def findMax(self):
		if(len(self.param_arr) == 1): return 0.
		phase_max =  phaseNearTargetPhaseDeg(-self.param_arr[2],0.)
		phase_step = 20.
		delta = 0.001
...
class HarmScorer(Scorer):
	...
	def setTrialParams(self,trial):
		...
		for i in range(1,len(self.param_arr),2):
			...
			self.param_arr[i+1] = phaseNearTargetPhaseDeg(self.param_arr[i+1],0.)
	...
	def getNextHarm_Amp_Shift(self):
		...
		phase_shift = phaseNearTargetPhaseDeg(n_harm*phase_shift,0.)
